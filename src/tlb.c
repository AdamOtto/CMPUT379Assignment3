...
Note: On a 32-bit machine, the above TLB entry requires two words. If
      your machine has 64-bit words, then you only need to use one word
*/

//On a 32-bit machine, you can use a struct to get a two-word entry.
typedef struct {
...
//select the various fields of a TLB entry (again, assuming 
//you are using a 32-bit word computer).
...
#define RBIT_MASK   0x40000000  //RBIT is second leftmost bit of second word
